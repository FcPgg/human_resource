 ...
 */
public class Base64FileUploadServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	private static Logger logger = Logger.getLogger(Base64FileUploadServlet.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<SysAttachments> attList = new ArrayList<SysAttachments>();
		...
		try {
			...
			if(params != null && params.size() > 0){
				...
				for(Map<String, String> param : params){
					...
					att.setSrcFileName(param.get("fileName"));
					att.setSrcFileType(param.get("fileType"));
					
					att.setUuid(MD5Util.MD5(String.valueOf(att.hashCode())));
					
					GenerateImage(param.get("image"), prop.getProperty("file.upload"), att.getUuid(), att.getSrcFileType());
					service.update(att);
					attList.add(att);
				}
